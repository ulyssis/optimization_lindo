...
//	nErrorCode = LSsolveMIP(pModel, NULL); //P491 manual
	...
	remove("matrixB");
	FILE *matrixB;
	matrixB = fopen("/home/li/work/dev/DiCAPS/2011OtcGameOptReform/codes_cameraready/matrixB", "w");
		{
        int nLinearity;
		...
		for (pt=0; pt< n*c; pt++)
			...
			if (pt % c == c-1)
				...
				printf("%f \t", primal[pt]);
				fprintf(matrixB, "%f\t", primal[pt]); //write to file matrixB in matlab's folder
				}
			}
