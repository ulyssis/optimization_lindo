...
#define APIVERSION \
	{\
	   ...
	   printf("\nLINDO API Version %s built on %s\n",szVersion,szBuild);\
		...
		int indexInarray_power(int n, int c, int i, int k)
			{
			int m = n*n + (i)*c + k;
			return m;
			}
...
int main()
    ...
    LSreadModelParameter(pModel,"lindo.par");
		...
		int num_constants = n*n + n*c + 3; // n*n pathloss, n*c power, 1 noise, 1 signalfade, 1 constant(1)
...
//	    fp = fopen("/home/li/work/tools/lindo/lindoapi/samples/c/dica/Gtilde.txt", "rb");
		...
		printf ("Gtilde:\n");
	  	...
	  	// read data from power.txt to array power
	  	double power[n*c+2];
//	  	fp = fopen("/home/li/work/tools/lindo/lindoapi/samples/c/dica/power.txt", "rb");
		...
		/*
		 * min:
		 * \sum x_{i,k}* x_{j,k}* h_{i,j}* p_{i,k}/p_{j,k},
		 * j!=i, k\in \mathcal{C}
		 */
		...
		for(i=0; i<n; i++)
			...
			for(j=0; j<n; j++ )
				...
				if( j!= i )
					{
					attenu = indexInarray_pathloss(n, i, j);
					for(k=0; k<c; k++)
						{
						pik = indexInarray_power(n, c, i, k);
						pjk = indexInarray_power(n, c, j, k);
						...
						code[ikod++]= attenu;			 /* g_{i,j} */
						code[ikod++]= EP_PUSH_NUM;
						code[ikod++]= pjk;				 /* p_{i,k} */
